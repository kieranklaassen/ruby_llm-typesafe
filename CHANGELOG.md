...
- `RubyLLM::Protocols::SystemOne`, the wire format: the latest user message (or an explicit `state` from `with_provider_options`) is the state, JSON object or array messages become structured state, and the typed `answers` map comes back through `Message#parsed` with the response model, token usage, and raw HTTP response.
...
- Explicit "doesn't support" errors for plain chat, arbitrary JSON Schemas, streaming, tools, and attachments: TypeSafe is structured output only.
